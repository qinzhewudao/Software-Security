...
char pattern[100][100]={"he","hers","his","she"};
char a[]="shers";
//申请新节点并初始化
TreeNode *getNewNode()
{
...
int searchAC(Tree root,char* str,int len)
{
    ...
    while(i < len)
    {
        ...
        if (tmp->next[pos]!=NULL)
        {
            ...
            {
                cout<<i-strlen(pattern
                               [tmp->patterNo])+1<<'\t'<<tmp->patterNo<<'\t'<<pattern[tmp->patterNo]<<endl;
            }
            i++;
        }
        else
...
int main()
{
    Tree root=buildingTree();   ///建立Trie字典树
    buildingFailPath(root);     ///添加failure表
    cout<<"待匹配字符串为："<<a<<endl;
    cout<<"模式串为："<<" "<<pattern[0]<<" "<<pattern[1]<<" "<<pattern[2]<<" "<<pattern[3]<<" "<<endl<<endl;
